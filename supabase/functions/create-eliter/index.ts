...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (createAuthError) {
      return new Response(JSON.stringify({ error: 'Erro ao criar usuário', details: createAuthError.message }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
    ...
    if (profileError) {
      return new Response(JSON.stringify({ error: 'Erro ao atualizar perfil', details: profileError.message }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
