...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const failEvaluation = async (message: string) => {
      await supabase.from('ai_evaluations').update({ status: 'error', error_message: message }).eq('id', evalId);
    ...
    if (answers && answers.length > 0) {
      ...
      const { data: questions } = await supabase.from('form_questions').select('*').in('id', questionIds);
      const { data: options } = await supabase.from('form_question_options').select('*').in('question_id', questionIds);
      ...
      for (const ans of answers) {
        ...
        if (q.question_type === 'open_ended') {
          ...
          if (ans.selected_option_ids) {
            ...
            const selectedOpts = options?.filter((o: any) => optIds.includes(o.id)).map((o: any) => o.option_text);
    ...
    try {
      const fetchAI = async () => {
        ...
        while (attempt < 2) {
          ...
          if (res.status === 429) {
            if (attempt === 0) { await new Promise(r => setTimeout(r, 3000)); attempt++; continue; }
            throw new Error("Limite de requisicoes da IA atingido. Tente novamente em alguns minutos.");
          } else if (res.status === 500 || res.status === 503) {
            if (attempt === 0) { await new Promise(r => setTimeout(r, 2000)); attempt++; continue; }
    ...
    try {
      ...
      if (match) {
        try { parsedResult = JSON.parse(match[0]); } catch (e2) {}
      }
    }

    if (!parsedResult || typeof parsedResult.overall_score !== 'number' || parsedResult.overall_score < 0 || parsedResult.overall_score > 100 || !parsedResult.justification) {
