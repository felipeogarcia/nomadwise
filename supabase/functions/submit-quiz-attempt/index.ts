...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (currentAttempts >= quiz.max_attempts) {
      return new Response(JSON.stringify({ error: 'Voce ja usou todas as suas tentativas para esta prova.' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
    ...
    if (missingAnswers || questionIds.length !== answeredQuestionIds.length || hasInvalidAnswers) {
      return new Response(JSON.stringify({ error: 'Preencha todas as respostas antes de enviar.' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
    ...
    const percentage = total_questions > 0 ? Math.round((auto_graded_correct / total_questions) * 100) : 0;
    ...
    let message = '';
    if (needs_review) {
      message = "Sua prova foi enviada! As perguntas objetivas foram corrigidas automaticamente. As perguntas abertas serao avaliadas pelo seu gestor. Voce sera notificado quando a correcao estiver concluida.";
    } else if (passed) {
    ...
    return new Response(JSON.stringify({ error: 'Erro interno ao processar a prova.', details: error.message }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    });
  }
