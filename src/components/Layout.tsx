...
import { Compass, LayoutDashboard, Map, Wrench, ShieldAlert, LogOut, Bell } from 'lucide-react'
import {
  Sidebar,
  SidebarContent,
  SidebarFooter,
...
export default function Layout() {
  ...
  const navItems = [
    ...
    { title: 'Garagem', url: '/app/garage', icon: Wrench },
  ]
      ...
      >
        <header className="sticky top-0 z-30 flex h-16 shrink-0 items-center justify-between gap-2 border-b bg-background/95 px-4 backdrop-blur supports-[backdrop-filter]:bg-background/60 shadow-sm">
          <div className="flex items-center gap-2">
            {!isPending && <SidebarTrigger className="-ml-1" />}
            {isPending && (
